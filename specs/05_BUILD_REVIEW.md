...
- **Verification:** final `pnpm check` passed on 2026-07-26 — traceability through Phase 3 (34 requirements), `tsc --noEmit` clean, Biome clean over 56 files, **126 unit tests** across 8 files (31 new in `packages/core/src/blocking.test.ts`), all package builds, size budgets, **18 E2E tests**, and **5 accessibility tests** with zero serious/critical violations on the two new fixtures.
- **Measured size:** core IIFE **7.18 kB gzip** against the 8 kB limit (5.77 kB in Phase 2, so blocking cost 1.41 kB and 0.82 kB of headroom remains). core+ui 7.49 kB / 15 kB; bridge 318 B / 4 kB; head snippet 714 B / 1.5 kB.
